...
#define EB_CLICK 500	// таймаут накликивания
...
#elif defined(AVR)
	volatile uint8_t *_pin_reg = portInputRegister(digitalPinToPort(pin));
	volatile uint8_t _bit_mask = digitalPinToBitMask(pin);
...
public:
			...
			if (_btnState) {                                                	// кнопка нажата
				if (!_readFlag(3)) {                                          	// и не была нажата ранее
					...
					if (debounce > EB_CLICK) {									// кнопка нажата после EB_CLICK
						_count = 0;												// сбросить счётчик и флаг кликов
						_clrFlag(5);
					}
				...
				if (_readFlag(3)) {                                           	// но была нажата
					...
					_debTimer = thisMls;                                        // сброс таймаута
				} else if (_count > 0 && debounce > EB_CLICK) _setFlag(5);		// флаг на клики
			}
		}
	}
	...
	bool isClick(byte clicks) {
		if (_count == clicks && _readFlag(5)) {
			_count = 0;
			return 1;
		}
		return 0;
